...
def train_esm_mol():
    ...
    parser.add_argument(
        "-T", "--Train", action='store_true', help="Training?"
    )
    parser.add_argument(
        "-I", "--Infer", action='store_true', help="Inference?"
    )
    parser.add_argument(
        "-C", "--Check", type=Path, required=False, default='default.ckpt'
    )
    ...
    if args.Infer==True:
        ...
        Do inference
        '''
        projhead = EsmMolProjectionHead.load_from_checkpoint(args.Check, projection_1=projection_1, projection_2=projection_2, projection_3=projection_3, loss_fn=lossfn, base_lr=args.lr)
        print(projhead)
